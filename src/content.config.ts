...
const professions = defineCollection({
  ...
  schema: z.object({
    ...
    title: z.string(),
    /**
     * Правило подбора профессии по итогам квиза (ТЗ №4).
     *
     * Тег — точное совпадение с названием чипа или карточки любого из трёх
     * шагов конструктора: направление (шаг 1), задача (шаг 2), уровень опыта
     * (шаг 3). Пул общий, откуда именно взят тег — не хранится: правило
     * от этого не зависит, а совпадения названий между шагами разводит
     * админка подписями «Направление: …» / «Задача: …» / «Уровень: …».
     *
     * Профессия показывается, если отмечены ВСЕ обязательные теги
     * и не меньше matchMin необязательных.
     */
    tags: z
      .array(
        z.object({
          tag: z.string(),
          /** Без флажка тег идёт в зачёт порога matchMin */
          required: z.boolean().default(false),
        }),
      )
      .default([]),
    /** Сколько необязательных тегов должно совпасть помимо всех обязательных */
    matchMin: z.number().default(0),
    /**
